...
const ProductItem = ({ id, image, name, price }) => {
    ...
    const isWishlisted = wishlistItems[id] ? true : false;
        ...
        >
            <div className='relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden border border-gray-100 hover:border-accent-200'>
                ...
                <div className='relative aspect-[4/5] overflow-hidden rounded-t-2xl bg-gray-100'>
                    ...
                    <img
                        src={image && image.length > 0 ? image[0] : 'https://via.placeholder.com/400x500?text=No+Image'}
                        alt={name}
                        className={`w-full h-full object-cover transition-all duration-700 group-hover:scale-110 ${imageLoaded || !(image && image.length > 0) ? 'opacity-100' : 'opacity-0'
                            }`}
                    ...
                    <div className={`absolute top-2 right-2 sm:top-4 sm:right-4 transition-all duration-300 ${isHovered || isWishlisted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'
                        } hidden xs:block`}>
                        <button
                            onClick={toggleWishlist}
                            className={`w-8 h-8 sm:w-10 sm:h-10 ${isWishlisted ? 'bg-red-50' : 'bg-white/90'} backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-colors duration-200 group/btn`}
                        >
                            <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${isWishlisted ? 'text-red-500 fill-red-500' : 'text-gray-700'} group-hover/btn:text-accent-600 transition-colors duration-200`} fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                    ...
                    <div className={`absolute bottom-2 left-1/2 -translate-x-1/2 transition-all duration-300 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                        } hidden xs:block`}>
                ...
                <div className='p-3 sm:p-4 space-y-1.5 sm:space-y-2'>
                    ...
                    <div className='flex items-center justify-between'>
                        ...
                        <div className='flex items-center gap-0.5 sm:gap-1'>
                            {[...Array(5)].map((_, i) => (
                                <svg key={i} className={`w-2.5 h-2.5 sm:w-3 sm:h-3 ${i < 4 ? 'text-yellow-400' : 'text-gray-300'
                ...
                {/* Shine effect */}
                <div className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -skew-x-12 transition-transform duration-1000 ${isHovered ? 'translate-x-full' : '-translate-x-full'
                    }`}></div>
            </div>
        </Link>
    )
